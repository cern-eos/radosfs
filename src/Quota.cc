...
int
QuotaPriv::create(PoolSP pool, const std::string &name, int64_t maxSize)
{
  this->pool = pool;
  this->name = name;
  ...
  omap[XATTR_QUOTA_CURRENT_SIZE].append("0");
  ...
  return pool->ioctx.operate(name, &op);
}

int
QuotaPriv::load(const std::string &name, PoolSP pool)
{
  if (!this->pool || (this->pool.get() != pool.get()))
    this->pool = pool;
  ...
  std::map<std::string, librados::bufferlist> omap;
  int ret = this->pool->ioctx.omap_get_vals(name, "",
                                            XATTR_QUOTA_SIZE_PREFIX, UINT_MAX,
                                            &omap);
...
Quota::Quota(const Quota &quota)
  : mPriv(new QuotaPriv)
{
  mPriv->pool = quota.mPriv->pool;
  mPriv->quotaSize = quota.mPriv->quotaSize;
  mPriv->name = quota.mPriv->name;
...
Quota::update()
{
  ...
  return mPriv->load(mPriv->name, mPriv->pool);
}
